...
import { RowType, ValueType } from 'typings/data/table';
...
const HeaderConfig: FC<HeaderConfigProps> = ({ onBack }) => {
  ...
  const changeDataIndex = (value?: string) => {
    ...
    handleTableState({ dataSource: newDataSource });
      ...
      >
            ...
            {isEditingIndex ? (
              ...
              dataSourceType === 'mock' ? (
                ...
                <div>
                  ...
                  >
                    {Object.entries(previewData[0])?.map((entry) => {
        ...
        <PanelLayout title={'数据类型'}>
          <Radio.Group
            ...
            onChange={(e) => {
              const valueType: ValueType = e.target.value;
          ...
          >
            ...
            <Radio className={styles.radio} value={'enum'}>
              枚举{' '}
              <InfoNote
                title={'固定某些值的文本类型 例如: [男,女] [前端,后端,设计] 等'}
              />
            </Radio>
        ...
        <DataFillPanel column={column} columnIndex={index} />
      </CollapsePanel>
    </div>
  );
};
